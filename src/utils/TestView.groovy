...
class TestView implements Serializable {
    ...
    def getOrCreateProject(text) {
        ...
        if (result.title.contains(text)){
            ...
            def setproject = steps.httpRequest url: "${XLTV_HOST}/api/v1/projects", authentication: "${authentication}", contentType: "${contentType}", acceptType: "${contentType}", httpMode: 'POST', requestBody: '{"title":"${pro}"}'
